...
```{r examplecomparison, message=FALSE, warning=FALSE}
# cd into directory for figures
setwd("CellChat_Plots")
setwd("Comparison")
...
rm(cellchat.NL, cellchat.LS)

# compare interactions
gg1 <- compareInteractions(cellchat1, show.legend = F, group = c(1,2))
...
netVisual_diffInteraction(cellchat1, weight.scale = T, measure = "weight")

# Differential number of interactions with heatmap
gg1 <- netVisual_heatmap(cellchat1)
gg2 <- netVisual_heatmap(cellchat1, measure = "weight")
gg1 + gg2
...
# Identify signaling groups based on their functional similarity
cellchat1 <- computeNetSimilarityPairwise(cellchat1, type = "functional")
cellchat1 <- netEmbedding(cellchat1, type = "functional")
cellchat1 <- netClustering(cellchat1, type = "functional")
# Visualization in 2D-space
netVisual_embeddingPairwise(cellchat1, type = "functional", label.size = 3.5)
# netVisual_embeddingZoomIn(cellchat1, type = "functional", nCol = 2)

# Identify signaling groups based on structure similarity
cellchat1 <- computeNetSimilarityPairwise(cellchat1, type = "structural")
cellchat1 <- netEmbedding(cellchat1, type = "structural")
cellchat1 <- netClustering(cellchat1, type = "structural")
# Visualization in 2D-space
netVisual_embeddingPairwise(cellchat1, type = "structural", label.size = 3.5)
netVisual_embeddingPairwiseZoomIn(cellchat1, type = "structural", nCol = 2)

# Compute and visualize the pathway distance in the learned joint manifold
rankSimilarity(cellchat1, type = "functional")

# Compare the overall information flow of each signaling pathway
gg1 <- rankNet(cellchat1, mode = "comparison", stacked = T, do.stat = TRUE)
gg2 <- rankNet(cellchat1, mode = "comparison", stacked = F, do.stat = TRUE)
gg1 + gg2
