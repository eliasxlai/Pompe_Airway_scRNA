...
count_raw <- gaa_merge_withoutdoublets@raw.data[,gaa_merge_withoutdoublets@cell.names]
count_norm <- apply(count_raw, 2, function(x) (x/sum(x))*10000)
write.table(count_norm, "cellphonedbfiles/cellphonedb_count.txt", sep="\t", quote=F)
# Generating metadata file.
meta_data <- cbind(rownames(gaa_merge_withoutdoublets@meta.data), 
                   gaa_merge_withoutdoublets@meta.data[,"cluster", drop=F])
# Cluster is the user’s corresponding cluster column.
write.table(meta_data, "cellphonedbfiles/cellphonedb_meta.txt", sep="\t", quote=F,
row.names=F)
```

```{r cellphonedb2, message=FALSE, warning=FALSE}
# Generating metadata file.
meta_data <- cbind(rownames(merge@meta.data), merge@meta.data[, "seurat_clusters", drop=F])
# Cluster is the user’s corresponding cluster column.
write.table(meta_data, "cellphonedbfiles/cellphonedb_meta.txt", sep="\t", quote=F,
row.names=F)
```

```{r cellphonedb3, message=FALSE, warning=FALSE}
...
write.table(matrix1[,s1], "cellphonedbfiles/s1_filtered_hcount.csv",row.names=T,sep=',')
write.table(matrix1[,s2], "cellphonedbfiles/s2_filtered_hcount.csv",row.names=T,sep=',')

# generate metadata
merge <- StashIdent(merge, save.name = "clusters")
alldata <- StashIdent(alldata, save.name = "clusters")
