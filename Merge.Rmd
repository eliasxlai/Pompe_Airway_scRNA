...
seven.markers.markers <- readRDS("output/seven.markers.rds")
...
endothelial <- subset(x = merge, idents = c("Endothelial", "Endothelial2"))
...
```{r}
seven.pos.markers <- FindMarkers(merge, ident.1 = "7", only.pos = TRUE, min.pct = 0.20)
```

```{r}
EnhancedVolcano(type2pneumocyte.markers,
                title = "GAA KO vs Control, Type 2 Pneumocyte",
                #  selectLab = m$V1,
                lab = rownames(type2pneumocyte.markers),
                x = 'avg_log2FC',
                y = 'p_val_adj',
...
EnhancedVolcano(type1pneumocyte.markers,
                title = "GAA KO vs Control, Type 1 Pneumocyte",
                # selectLab = c("Pkp3"),
                lab = rownames(type1pneumocyte.markers),
                x = 'avg_log2FC',
                y = 'p_val_adj',
...
```{r type1pneumocyte-gene-identification}
type1pneumocyte.markers.filtered <- type1pneumocyte.markers %>% 
  filter(p_val_adj <= 10e-2) %>% 
  filter(avg_log2FC >= 0.35)
# FeaturePlot(merge, c("Rps19"), split.by = "condition")

setwd("FeaturePlots_from_volcano_genes")
setwd("type1pneumocyte")
genes <- row.names(type1pneumocyte.markers.filtered)
for (gene in genes) {
  gg <- FeaturePlot(merge, gene, split.by = "condition")
  ggsave(filename=paste0(gene, "_type1pneumocyte.jpeg"), plot=gg, width = 24, height = 16, units = 'in', dpi = 300, device = "jpeg")
}
setwd("..")
setwd("..")
```

```{r type2pneumocyte-gene-identification}
type2pneumocyte.markers.filtered <- type2pneumocyte.markers %>% 
  filter(p_val_adj <= 10e-2) %>% 
  filter(avg_log2FC >= 0.35)
# FeaturePlot(merge, c("Rps19"), split.by = "condition")

setwd("FeaturePlots_from_volcano_genes")
setwd("type2pneumocyte")
genes <- row.names(type2pneumocyte.markers.filtered)
for (gene in genes) {
  gg <- FeaturePlot(merge, gene, split.by = "condition")
  ggsave(filename=paste0(gene, "_type2pneumocyte.jpeg"), plot=gg, width = 24, height = 16, units = 'in', dpi = 300, device = "jpeg")
}
setwd("..")
setwd("..")
```
...
VlnPlot(clean_merge, "Ndnf")

FeaturePlot(type1pneumocyte, "Ndnf", split.by = "condition")
VlnPlot(merge, "Clic3", split.by = "condition")

FeaturePlot(type2pneumocyte, "Mgp", split.by = "condition")
VlnPlot(merge, "Neat1", split.by = "condition", pt.size = 0)
```

```{r}
merge_rm_clusters <- c(7, 14)
clean_merge <- subset(x = merge, 
                                     idents = merge_rm_clusters, invert = TRUE)

clean_merge <- NormalizeData(clean_merge, normalization.method = "LogNormalize", scale.factor = 10000)
clean_merge <- FindVariableFeatures(clean_merge, selection.method = "vst", nfeatures = 2000)
all.genes <- rownames(clean_merge)
clean_merge <- ScaleData(clean_merge, features = all.genes)
clean_merge <- RunPCA(object = clean_merge, verbose = FALSE)
ElbowPlot(clean_merge, ndims = 30)
clean_merge <- FindNeighbors(object = clean_merge, dims = 1:20)
clean_merge <- FindClusters(object = clean_merge, resolution = 1)
clean_merge <- RunUMAP(object = clean_merge, dims = 1:20,min.dist = 1, n.neighbors = 40, spread = 5, local.connectivity = 20)
