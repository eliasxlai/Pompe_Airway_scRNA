...
```{r pathway_specific_heatmaps, message=FALSE, warning=FALSE}
# Identified pathways
identified_pathways <- c("EPHB", "CHEMERIN", "SEMA6", "CDH", "AGRN", "VTN", "BMP", "ANGPT", "SEMA7", "TRAIL", "ICAM", "PTN", "ITGAL-ITGB2", "GALECTIN", "KIT", "RELN", "ncWNT", "PERIOSTIN", "CDH1", "VISFATIN", "EGF", "PTPRM", "CD34")

# Heatmap
for (k in 1:length(identified_pathways)) {
  pathways.show <- c(identified_pathways[k]) 
  par(mfrow = c(1,2), xpd=TRUE)
  ht <- list()
  for (i in 1:length(object.list)) {
    ht[[i]] <- netVisual_heatmap(object.list[[i]], signaling = pathways.show, color.heatmap = "Reds",title.name = paste(pathways.show, "signaling ",names(object.list)[i]))
